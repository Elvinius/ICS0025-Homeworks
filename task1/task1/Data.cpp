...
Data::~Data()
{
	for (auto dit = datastructure.begin(); dit != datastructure.end(); ++dit) {
...
map<int, list<Item*>*>* Data::GetGroup(char group) const 
{
	...
	map<char, map<int, list<Item*>*>*>::const_iterator it = datastructure.find(group);
	it == datastructure.end() ? groupMap = nullptr : groupMap = it->second;
	return groupMap;
}
...
Item* Data::InsertItem(char group, int subg, string name, Date date)
{
	...
	if (subgroupList) {
		...
		if (subgroupMap) { 
			...
			subgroupMap->insert(make_pair(pItem->GetSubgroup(), subgroupList));
			datastructure.insert(make_pair(pItem->GetGroup(), subgroupMap));
		}
	}
	return pItem;
}
...
{
	map<char, map<int, list<Item*>*>*>::iterator groupIterator = datastructure.find(group);
	if (groupIterator != datastructure.end() || group < 'A' || group > 'Z' || subgroup.size() != items.size()) {
	...
	datastructure.insert(make_pair(group, pGroup));
...
bool Data::RemoveItem(char group, int subgroup, string name)
{
	...
	if (itemList->empty()) {
		...
		if (subgroupMap->empty()) {
			delete subgroupMap;
			datastructure.erase(datastructure.find(group));
		}
	}
