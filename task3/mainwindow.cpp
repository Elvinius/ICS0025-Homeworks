...
void MainWindow::on_c_Btn_clicked()
{
    ...
    if(local_socket->state() == QLocalSocket::ConnectedState){
        ...
        ui->log_txt_browser->append("Successfully connected to the pipe");
    }else{
...
void MainWindow::on_function_box_currentIndexChanged(int index)
{
    if(index == 2){
...
void MainWindow::on_com_Btn_clicked()
{
    if(local_socket->state() != QLocalSocket::ConnectedState){
...
void MainWindow::setup_everything()
{
    ui->d_btn->setEnabled(false);
    ui->c_Btn->setEnabled(true);
    ui->order_edit->setEnabled(false);
...
QByteArray MainWindow::create_wchar_from_string(QString string, int general_size)
{
...
QByteArray MainWindow::prep_packet_to_send()
{
